    ...
    if (conversation?.projectId) {
      try {
        ...
        if (project) {
          ...
          systemPrompt += `\n**ENVIRONMENT CONTEXT:**\n`;
          systemPrompt += `- Working Directory: ${project.localPath || '/workspace/projects/' + project.name.toLowerCase().replace(/[^a-z0-9]+/g, '-')}\n`;
          systemPrompt += `- All commands execute in this directory automatically (no cd needed)\n`;
          systemPrompt += `- All file paths are relative to this directory\n`;
          systemPrompt += `- Project name for package.json: "${project.name}"\n\n`;
          systemPrompt += `**CRITICAL RULES:**\n`;
          systemPrompt += `1. ALL operations happen in the current project directory\n`;
          systemPrompt += `2. When creating apps: Use "." as target (e.g., "npx create-react-app .")\n`;
          systemPrompt += `3. NEVER create subdirectories like "myapp/", "app/" - files go in project root\n`;
          systemPrompt += `4. For dev commands (npm start, npm run dev): They run in project directory\n`;
          systemPrompt += `5. File operations: Use relative paths (e.g., "src/App.tsx" not "/workspace/projects/myapp/src/App.tsx")\n`;
          systemPrompt += `6. Dev servers run indefinitely - use dev server management tools to control them\n\n`;
          systemPrompt += `**EXAMPLES:**\n`;
          systemPrompt += `✅ CORRECT: "npx create-react-app . --template typescript"\n`;
          systemPrompt += `✅ CORRECT: "npm install express" (installs in current project)\n`;
          systemPrompt += `✅ CORRECT: "npm run dev" (runs in current project)\n`;
          systemPrompt += `❌ WRONG: "npx create-react-app myapp" (creates unwanted subdirectory)\n`;
          systemPrompt += `❌ WRONG: "cd myapp && npm start" (unnecessary, already in project dir)\n\n`;
          systemPrompt += `**DEV SERVER MANAGEMENT:**\n`;
          systemPrompt += `Dev servers (npm start, npm run dev, etc.) run indefinitely. Use these tools:\n`;
          systemPrompt += `- stop_dev_server: Stop a specific dev server by terminal ID, port, or working directory\n`;
          systemPrompt += `- stop_all_dev_servers: Stop all running dev servers\n`;
          systemPrompt += `- check_dev_server: Check if a dev server is running on a specific port\n`;
          systemPrompt += `- list_dev_servers: List all running dev servers with their details\n\n`;
      ...
      systemPrompt += `- Working Directory: /workspace (default)\n`;
      systemPrompt += `- No project context available\n`;
      systemPrompt += `- All commands execute in /workspace\n\n`;
      systemPrompt += `**RECOMMENDATIONS:**\n`;
      systemPrompt += `1. For development work, suggest user creates a project in the UI first\n`;
      systemPrompt += `2. Projects provide better organization and context\n`;
      systemPrompt += `3. If user wants to create an app, you can work in /workspace temporarily\n`;
      systemPrompt += `4. Use app name in package.json if user provides one\n\n`;
    }
