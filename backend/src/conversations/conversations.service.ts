    ...
    if (conversation?.projectId) {
      try {
        ...
        if (project) {
          ...
          systemPrompt += `**CRITICAL WORKING DIRECTORY RULES - MUST FOLLOW:**\n`;
          systemPrompt += `1. You are working in project: "${project.name}"\n`;
          systemPrompt += `2. Project directory: ${project.localPath || '/workspace/projects/' + project.name.toLowerCase().replace(/[^a-z0-9]+/g, '-')}\n`;
          systemPrompt += `3. ALL commands execute in this directory automatically\n`;
          systemPrompt += `4. When user asks to "create a React app" or similar:\n`;
          systemPrompt += `   - DO NOT use "npx create-react-app myapp" or any subdirectory name\n`;
          systemPrompt += `   - USE "npx create-react-app ." to create in current directory\n`;
          systemPrompt += `   - OR manually create files (package.json, src/, public/) directly here\n`;
          systemPrompt += `5. The project name "${project.name}" should be used in package.json "name" field\n`;
          systemPrompt += `6. Files go directly in project root: src/, public/, package.json, etc.\n`;
          systemPrompt += `7. NEVER create subdirectories like "myapp/", "app/", etc. - everything goes in current directory\n\n`;
          systemPrompt += `**EXAMPLE - CORRECT:**\n`;
          systemPrompt += `User: "create a React app"\n`;
          systemPrompt += `You: npx create-react-app . --template typescript\n`;
          systemPrompt += `(This creates files directly in ${project.localPath})\n\n`;
          systemPrompt += `**EXAMPLE - WRONG:**\n`;
          systemPrompt += `User: "create a React app"\n`;
          systemPrompt += `You: npx create-react-app myapp  ← WRONG! Creates unwanted subdirectory\n\n`;
